...
class GNF5_Writer {
    ...
    public static function create_article($research, $cat_id) {
        ...
        for($attempt=0;$attempt<=$limit;$attempt++){
            ...
            $raw=isset($job['article'])?$job['article']:self::gemini_json($prompt);if(is_wp_error($raw))return $raw;
    ...
    public static function repair_for_validation($article, $research, $errors, $cat_id=0) {
        ...
        $repaired['plan']=$article['plan']??array();$repaired['short_reason']=$article['short_reason']??'';
