...
class GNF5_Writer {
    ...
    public static function create_article($research, $cat_id, $ignore_post=0) {
        ...
        for($attempt=0;$attempt<=$limit;$attempt++){
            ...
            $raw=isset($job['article'])?$job['article']:self::gemini_json($prompt."\nINDEPENDENT_HEADLINES:\n".wp_json_encode(array_intersect_key($titles,array_flip(array('title','seo_title'))))."\nALREADY_USED_FOCUS_KEYWORDS (choose a factual, article-specific phrase instead of reusing a broad phrase):\n".wp_json_encode(GNF5_SEO::recent_focus_keywords(200)).self::seo_instruction().self::text_repair_instruction());if(is_wp_error($raw))return $raw;
    ...
    public static function seo_instruction() {
        return "\nNON-IMAGE SEO: Choose one specific, natural focus keyword. Use it in the SEO title (preferably near the start), meta description, concise slug, opening 10 percent of the body and a relevant H2/H3. Aim near 1.5 percent exact-phrase density (roughly 1.3–1.7 percent), spread naturally through the body; remove repetitive wording rather than stuffing keywords. Aim for 1000–1200 words, at least 600 when evidence allows useful explanation, background or comparison. If evidence cannot support that length, return a clear short_reason and preserve factual accuracy. Use short paragraphs and useful headings. A sentiment word, power word or number in the title is welcome only if truthful and justified by the facts or actual list structure. Otherwise keep a neutral accurate title. Do not manufacture a table of contents claim. Never add links or images yourself; the plugin handles validated links and the image setting.\n";
    }

    public static function text_repair_instruction() {
        return "\nTEXT REPAIR PRIORITIES: First preserve every already-passing text check and the exact focus keyword unless KEYWORD_RULE explicitly permits a factual refinement of a reused phrase. Put that keyword naturally in the first sentence or opening paragraph, within the first 10 percent of body text; do not rely on a heading or table of contents. If the body is under 600 words, develop relevant verified facts into at least 600 words, preferably 1000–1200: explain scope, implications, limitations and useful comparisons only where the fact sheet supports them. Do not repeat paragraphs, pad the article or invent facts. If the evidence is insufficient, return short_reason identifying what evidence is missing. For a title without a number, use a relevant number explicitly supported by the fact sheet, or an accurate count of distinct items actually discussed in a useful list. Keep the focus keyword at the beginning of the SEO title and prioritize factual wording over sentiment/power words. Do not append an arbitrary year, change the story into a list merely for points, or invent a statistic. Preserve the keyword in the description, slug, body and headings; keep short paragraphs. Image settings and image metadata must remain unchanged.\n";
    }

    public static function repair_for_validation($article, $research, $errors, $cat_id=0,$post_id=0) {
        ...
        $repaired['original_focus_keyword']=$article['original_focus_keyword']??$article['focus_keyword'];
