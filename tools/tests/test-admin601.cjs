...
for(const failure of ['locked','network']){
  ...
  function $(s){if(typeof s==='function'){return;}return s instanceof Element?s:new Element(s);}
  $.post=(url,data)=>{
  ...
  assert.equal(calls,1);assert.doesNotMatch(msg,/run stopped/);assert.doesNotMatch(msg,/No usable|run finished/);
